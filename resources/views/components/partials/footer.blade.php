      ...
      <div class="row">
        ...
        <div class="col-lg-4  col-md-6 col-sm-6">
          <div class="single-footer-widget">
            ...
            <form
                action="{{ route('subscriber.store') }}" method="POST" class="form-inline">
                @csrf
                <div class="d-flex flex-row">
                    <input class="form-control" name="email" value="{{ old('email') }}"  placeholder="Enter Email" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Email '"
                        required="" type="email">
                        <span class="info block text-success">{{ session('success') }}</span>
                    @error('email')
                        <div class="text-danger block">{{ $message }}</div>
                    @enderror
